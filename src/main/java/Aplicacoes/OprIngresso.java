...
public class OprIngresso {
    ...
    private double calcularValorReal(String tipo, double valorBase) {
        if (tipo.equalsIgnoreCase("VIP"))  return valorBase * 2;
        if (tipo.equalsIgnoreCase("Meia")) return valorBase / 2;
        return valorBase;
    }
    ...
    public void venderIngresso(String tipo, Partida partida, double valorBase, int quantidade) throws Exception {
        ...
        if (vendidosNaPartida + quantidade > capacidade) {
            throw new Exception("Capacidade esgotada. Vendidos: " + vendidosNaPartida + " | Disponíveis: " + (capacidade - vendidosNaPartida) + " | Capacidade: " + capacidade);
        }

        double valorReal = calcularValorReal(tipo, valorBase);
    ...
    private boolean mesmaPartida(Partida a, Partida b) {
        ...
        return a.getData().equalsIgnoreCase(b.getData()) && a.getHora().equalsIgnoreCase(b.getHora()) && a.getTimeCasa().getPais().equalsIgnoreCase(b.getTimeCasa().getPais()) && a.getTimeVisita().getPais().equalsIgnoreCase(b.getTimeVisita().getPais());
    ...
    public void devolverIngresso(String tipo, Partida partida, int quantidade) throws Exception {
        ...
        if (disponiveis < quantidade) {
            throw new Exception("Não há ingressos suficientes do tipo " + tipo + " para devolver. Disponíveis: " + disponiveis);
        }
    ...
    public int contarIngressosVendidosPorPartidaETipo(Partida partida, String tipo) {
        int total = 0;
        for (Ingresso ingresso : ingressosVendidos) {
            if (mesmaPartida(ingresso.getPartida(), partida) && ingresso.getTipo().equalsIgnoreCase(tipo)) {
                total++;
            }
        }
        return total;
    ...
    public void salvarDadosNoArquivo() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(CAMINHO_ARQUIVO))) {
            for (Ingresso ing : ingressosVendidos) {
                ...
                writer.write("INGRESSO;" + ing.getTipo() + ";" + ing.getValor() + ";" + p.getTimeCasa().getPais() + ";" + p.getTimeVisita().getPais() + ";" + p.getData() + ";" + p.getHora() + ";" + p.getEstadio().getNome() + ";" + p.getEstadio().getLocal() + ";" + p.getEstadio().getVagas() + ";" + p.getStatus());
                writer.newLine();
